...
public class SatelliteEventListener {
    ...
    @KafkaListener(topics = KafkaTopics.SATELLITE_CREATED, groupId = "mission-service")
    public void onSatelliteCreated(SatelliteEvent event) {
        log.info("Получено событие создания спутника: {}", event);
        satelliteRegistry.register(event);
    }

    @KafkaListener(topics = KafkaTopics.SATELLITE_DELETED, groupId = "mission-service")
    public void onSatelliteDeleted(SatelliteEvent event) {
        log.info("Получено событие удаления спутника: {}", event);
